...
public:
  ...
  void Advection(void);
  void Derivatives(double dt);
  double sigmaxx(double rho0, double eta, double dx_dx);
  double sigmaxy(double eta, double dx_dy, double dy_dx);
...
void LatticeBoltzmann::Derivatives(double dt){
   int ix,iy,i,ixnext,iynext;
   double rho0,Ux0,Uy0;
   ...
   for(ix=0;ix<Lx;ix++) //for each cell
    for(iy=0;iy<Ly;iy++)
      for(i=0;i<Q;i++){ //on each direction
	ixnext=(ix+Vx[i]+Lx)%Lx; iynext=(iy+Vy[i]+Ly)%Ly;
	rho0=rho(ixnext,iynext,false); Ux0=Jx(ixnext,iynext,false)/rho0; Uy0=Jy(ixnext,iynext,false)/rho0;
	...
	double dx_dx = (3/dt)*sumxx;
        double dx_dy = (3/dt)*sumxy;
	double dy_dx = (3/dt)*sumyx;
	double dy_dy = (3/dt)*sumyy;
  }
}
...
double LatticeBoltzmann::sigmaxy(double eta, double dx_dy, double dy_dx){
